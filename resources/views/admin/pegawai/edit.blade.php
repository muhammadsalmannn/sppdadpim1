                    ...
                    <form action="/pegawai/{{ $pegawais[0]->id }}" method="post">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text"
                                ...
                                is-invalid
                            @enderror"
                                id="nama" name="nama" placeholder="Masukan Nama..." autofocus required
                                value="{{ old('nama', $pegawais[0]->nama) }}">
                            @error('nama')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text"
                                ...
                                is-invalid
                            @enderror"
                                id="nip" name="nip" placeholder="Masukan Nip..." required
                                value="{{ old('nip', $pegawais[0]->nip) }}">
                            @error('nip')
                                <div class="invalid-feedback">
                                    {{ $message }}
                                </div>
                            ...
                            <select class="form-select" name="pangkat_id">
                                @foreach ($pangkats as $pangkat)
                                    @if (old('pangkat_id', $pegawais[0]->pangkat_id) == $pangkat->id)
                                        <option value="{{ $pangkat->id }}" selected>
                                            {{ $pangkat->golongan }}/{{ $pangkat->ruang }}</option>
                                    @else
