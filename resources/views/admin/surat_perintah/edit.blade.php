...
@section('container')
    ...
    <div class="container container-fluid d-flex justify-content-center">
        <div class="col-md-8 ">
            <div class="portlet light bordered">
                        ...
                        <select class="form-select" name="perjalanan_id">
                            @foreach ($perjalanans as $perjalanan)
                                @if (old('perjalanan_id', $surats[0]->perjalanan_id) == $perjalanan->id)
                                    <option value="{{ $perjalanan->id }}" selected>{{ $perjalanan->tempat_tujuan }}
                                    </option>
                                @else
                ...
                <input type="text"
                    class="form-control @error('nomor')
                                is-invalid
                            @enderror"
                    id="nomor" name="nomor" placeholder="Masukan Nomor SPPD..." autofocus required
                    value="{{ old('nomor', $surats[0]->nomor) }}">
                @error('nomor')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
            ...
            <div class="mb-3">
                <label for="uraian" class="form-label">Maksud Perjalanan</label>
